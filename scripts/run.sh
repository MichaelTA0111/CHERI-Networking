...
# Usage: ./run.sh {build_dir_name} {packet_stream} {opts}
...
"$build_dir"/examples/dpdk-cheri_networking -l 0 -n 4 --no-huge --no-shconf --vdev=net_pcap0,rx_pcap=packet_streams/"$packet_stream",tx_pcap=packet_streams/out.pcap -- -"$opts" 2> /dev/null
